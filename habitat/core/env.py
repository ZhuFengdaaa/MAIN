...
class Env:
    ...
    def __init__(
        ...
        iter_option_dict["seed"] = config.GROUP_SEED
    ...
    def forced_scene_switch(self):
        self._episode_iterator._forced_scene_switch()
        self._episode_iterator._set_shuffle_intervals()

        if self._current_episode is not None and self._current_episode.episode_id is not None:
            before_id = self._current_episode.episode_id
            (self._current_episode, after_id) = self._episode_iterator._next(scene_switch_if=False)
            logger.info(f"forced_scene_switch: reset from {before_id} to {after_id}")
        else:
            (self._current_episode, _) = self._episode_iterator._next(scene_switch_if=False)
        
        return self.reset(iter_next=False)

    def reset(self, iter_next=True) -> Observations:
        ...
        if iter_next:
            if self._current_episode is not None and self._current_episode.episode_id is not None:
                before_id = self._current_episode.episode_id
                (self._current_episode, after_id, do_switch) = self._episode_iterator._next()
                # logger.info(f"do_switch: {do_switch}, reset from {before_id} to {after_id}")
            else:
                (self._current_episode, _, do_switch) = self._episode_iterator._next()
        ...
        info = self._task.measurements.get_metrics()
        ...
        return observations, do_switch, info
    ...
    def seed(self, seed: int, seed2:int = None) -> None:
        self._sim.seed(seed)
        if seed2 is None:
            self._task.seed(seed)
        else:
            self._task.seed(seed2)
...
class RLEnv(gym.Env):
    ...
    def forced_scene_switch(self):
        return self._env.forced_scene_switch()
