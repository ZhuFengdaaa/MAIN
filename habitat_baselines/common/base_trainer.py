...
class BaseRLTrainerOracle(BaseTrainer):
    ...
    def __init__(self, config: Config):
        ...
        if self.config.TASK_CONFIG.ENVIRONMENT.MULTI_AGENT:
            self.multi_agent = True
            self.agent_num = self.config.TASK_CONFIG.ENVIRONMENT.AGENT_NUM
            self.multi_agent_type = self.config.TASK_CONFIG.ENVIRONMENT.MULTI_AGENT_TYPE
        else:
