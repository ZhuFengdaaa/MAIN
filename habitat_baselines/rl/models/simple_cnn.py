...
class RGBCNNOracle(nn.Module):
    ...
    def permute_input(self, _input):
        if len(_input.shape) == 4:
            return _input.permute(0, 3, 1, 2)
        elif len(_input.shape) == 5:
            return _input.permute(0, 1, 4, 2, 3)
        else:
    ...
    def forward(self, observations):
        ...
        if self._n_input_rgb > 0:
            rgb_observations = observations["rgb"]
            # permute tensor to dimension [BATCH x CHANNEL x HEIGHT X WIDTH]
            rgb_observations = self.permute_input(rgb_observations)
            rgb_observations = rgb_observations / 255.0  # normalize RGB
            cnn_input.append(rgb_observations)
        ...
        if self._n_input_depth > 0:
            depth_observations = observations["depth"]
            # permute tensor to dimension [BATCH x CHANNEL x HEIGHT X WIDTH]
            depth_observations = self.permute_input(depth_observations)
            cnn_input.append(depth_observations)

        if len(cnn_input[0].shape) == 4:
            cnn_input = torch.cat(cnn_input, dim=1)
            return self.cnn(cnn_input)
        elif len(cnn_input[0].shape) == 5:
            cnn_input = torch.cat(cnn_input, dim=2)
            _shape = cnn_input.shape
            batch_size, agent_num = _shape[0], _shape[1]
            return self.cnn(cnn_input.view(-1, *_shape[2:]))
        else:
            raise NotImplementedError
