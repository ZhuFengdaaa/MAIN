...
class MultiPolicyOracle(nn.Module):
    def __init__(self, agent_type, observation_space, action_space,
        goal_sensor_uuid, device, object_category_embedding_size,
        previous_action_embedding_size, use_previous_action,
        batch_size, agent_num, hidden_size=512, action_emb_size=16,
        state_comm=False
    ):
        super().__init__()
        self.net = MultiNetOracle(
            agent_type,
        ...
        if action_emb_size > 0:
            self.action_embedding = nn.Embedding(self.dim_actions, action_emb_size)
            self.QNet = CriticHead((self.net.output_size + action_emb_size) * agent_num, output_size=agent_num)
        else:
            # self.critic = CriticHead(self.net.output_size * agent_num, output_size=agent_num)
            self.critic = CriticHead(self.net.output_size, output_size=1)
    ...
    def act(
        ...
        value, action, action_log_probs = self.get_value_action(features, deterministic=deterministic)
        return value, action, action_log_probs, rnn_hidden_states

    def get_value_action(self, features, deterministic=False, pred_action=True):
        action = None
        ...
        bs = features.size(0)
        bs //= self.agent_num
        # features = features.view(bs, -1) # (T * BS) * (N * C)
        features = features.view(bs, self.agent_num, -1) # (T * BS) * N * C
        if self.action_emb_size > 0:
            action_embeddings = self.action_embedding(action)
            action_embeddings = action_embeddings.view(bs, -1)
            v_inputs = torch.cat([features, action_embeddings], dim=1)
            value = self.QNet(v_inputs)
        else:
            bs = features.size(0)
            v_inputs = features
            # v_inputs = features.view(bs, 1, -1).repeat(1, self.agent_num, 1)
            # value_metrix = self.critic(v_inputs)
            # values = torch.cat([value_metrix[:,i,i].unsqueeze(1) for i in range(self.agent_num)], dim=1)
            values = self.critic(v_inputs).squeeze()
    ...
    def get_value(self, observations, rnn_hidden_states, prev_actions, masks):
        ...
        value, _, _ = self.get_value_action(features, pred_action=False)
        return value
    ...
    def evaluate_actions(
        ...
        values, _, _ = self.get_value_action(features, pred_action=False)
        distribution = self.action_distribution(features)
        action_log_probs = distribution.log_probs(action)
        distribution_entropy = distribution.entropy().mean()
        # value = value.repeat(1, self.agent_num).view(-1, 1) # adapt action calculation

        return values, action_log_probs, distribution_entropy, rnn_hidden_states
...
class BaselineNetOracle(Net):
    ...
    def __init__(self, agent_type, observation_space, hidden_size, goal_sensor_uuid, device, 
        object_category_embedding_size, previous_action_embedding_size, use_previous_action, 
        batch_size=None, agent_num=None
    ):
        super().__init__()
...
class MultiNetOracle(Net):
    ...
    def __init__(self, agent_type, observation_space, hidden_size, goal_sensor_uuid, device, 
        object_category_embedding_size, previous_action_embedding_size, use_previous_action, 
        batch_size=None, agent_num=None, state_comm=False):
        super().__init__()
        self.agent_num = agent_num
        self.batch_size = batch_size
        self.agent_type = agent_type
        ...
        if self.use_previous_action:
            if self.state_comm:
                # self._hidden_size: 768, object_category_embedding_size: 32, previous_action_embedding_size: 
                self.state_encoder = RNNStateEncoder(
                    (self._hidden_size)*(self.agent_num+1) + object_category_embedding_size + 
                    previous_action_embedding_size, self._hidden_size,
                )
            else:
                self.state_encoder = RNNStateEncoder(
    ...
    def forward(self, observations, rnn_hidden_states, prev_actions, masks):
        ...
        # comunication
        if self.state_comm:
            bs = rnn_hidden_states.size(1)
            assert x.size(0) % bs == 0 and bs % self.agent_num == 0
            num_iter = x.size(0) // bs
            c = x.size(1)
            x_slice = x.view(-1, bs, c)
            mask_slice = masks.view(-1, bs, 1)
            _x_slice = []
            for i in range(num_iter):
                ...
                # try:
                #     assert x.size(0) == rnn_hidden_states.size(1)
                # except:
                #     import ipdb; ipdb.set_trace()
                state_comm = rnn_hidden_states.view(bs // self.agent_num, self.agent_num, -1).detach()
                state_comm = state_comm.repeat(1, self.agent_num, 1)
                state_comm=state_comm.view(bs, -1)
                try:
                    _x = torch.cat([_x, state_comm], dim=1)
                except:
                    import ipdb; ipdb.set_trace()
                try:
                    _x, rnn_hidden_states = self.state_encoder(_x, rnn_hidden_states, _masks)
                except:
                    import ipdb; ipdb.set_trace()
                _x_slice.append(_x)
            x = torch.cat(_x_slice,dim=0)
        else:
            x, rnn_hidden_states = self.state_encoder(x, rnn_hidden_states, masks)
