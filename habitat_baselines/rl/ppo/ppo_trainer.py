...
from habitat.utils.visualizations.utils import observations_to_image, color_top_down_map
...
from habitat_baselines.common.rollout_storage import RolloutStorageOracle, RolloutStorageNonOracle, MultiRolloutStorageOracle
...
from habitat_baselines.rl.ppo import PPONonOracle, PPOOracle, BaselinePolicyNonOracle, BaselinePolicyOracle, MultiPolicyOracle
...
class AssertInconsistentError(Exception):
    pass
...
class PPOTrainerO(BaseRLTrainerOracle):
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config) -> None:
        ...
        #     observation_space=self.envs.rollout_observation_spaces,
        #     action_space=self.envs.rollout_action_spaces,
        #     hidden_size=ppo_cfg.hidden_size,
        #     goal_sensor_uuid=self.config.TASK_CONFIG.TASK.GOAL_SENSOR_UUID,
        #     device=self.device,
        #     object_category_embedding_size=self.config.RL.OBJECT_CATEGORY_EMBEDDING_SIZE,
        ...
        # )
        self.actor_critic = MultiPolicyOracle(
            agent_type = self.config.TRAINER_NAME,
            observation_space=self.envs.rollout_observation_spaces,
            action_space=self.envs.rollout_action_spaces,
            hidden_size=ppo_cfg.hidden_size,
            ...
            batch_size = self.batch_size,
            agent_num = self.agent_num,
            action_emb_size = self.config.RL.PPO.action_emb_size, 
            state_comm = self.config.RL.PPO.state_comm
        )
        self.actor_critic.to(self.device)
    ...
    METRICS_BLACKLIST = {"top_down_map", "collisions.is_collision", "raw_metrics", "traj_metrics", "foundDistance"}
    ...
    def _collect_rollout_step(
        ...
        with torch.no_grad():
            ...
            ### reshape in policy
            # for k, v in step_observation.items():
            #     _shape = step_observation[k].shape
            #     assert _shape[0] % self.batch_size == 0
            #     step_observation[k] = step_observation[k].view(self.batch_size, 
            #         _shape[0]//self.batch_size, *_shape[1:])
        ...
        outputs, rewards, global_dones, check_success = self.envs.step([a[0].item() for a in actions])
        if not check_success:
            raise AssertInconsistentError
        # observations, rewards, dones, infos = [list(x) for x in zip(*outputs)]
        observations, _, dones, infos = [list(x) for x in zip(*outputs)] # overwrite rewards
        # try:
        #     assert np.all(np.abs(rewards - np.array(_)) < 0.0001) == True
        # except:
        #     import ipdb; ipdb.set_trace()
        # logger.info(f"global reward {rewards}")
        # logger.info(f"local reward {_}")
        ...
        global_dones = torch.tensor(global_dones, dtype=torch.float, device=current_episode_reward.device).unsqueeze(1)
        global_masks = 1 - global_dones
        masks = global_masks.repeat(1, self.agent_num).reshape(-1, 1)
        current_episode_reward += rewards
        running_episode_stats["reward"] += (1 - global_masks) * current_episode_reward
        running_episode_stats["count"] += 1 - global_masks
        for k, v in self._extract_scalars_from_infos(infos).items():
            v = torch.tensor(
                v, dtype=torch.float, device=current_episode_reward.device
            ).unsqueeze(1)
            if k not in running_episode_stats:
                running_episode_stats[k] = torch.zeros_like(v)
            if v.shape == masks.shape:
                running_episode_stats[k] += (1 - masks) * v
            elif v.shape == global_masks.shape:
                running_episode_stats[k] += (1 - global_masks) * v
            else:
                raise NotImplementedError
                # import ipdb; ipdb.set_trace()
        current_episode_reward *= global_masks
    ...
    def reset_env(self, ppo_cfg):
        ...
        rollouts = MultiRolloutStorageOracle(
            ppo_cfg.num_steps,
            self.envs.batch_size,
            self.envs.agent_num,
            self.envs.rollout_observation_spaces,
            self.envs.rollout_action_spaces,
            ppo_cfg.hidden_size,
            state_comm=ppo_cfg.state_comm
        )
        rollouts.to(self.device)
    ...
    def train(self) -> None:
        ...
        multi_agent_flag = self.config.TASK_CONFIG.ENVIRONMENT.MULTI_AGENT
        
        rollouts = self.reset_env(ppo_cfg)

        if not os.path.isdir(self.config.CHECKPOINT_FOLDER):
            os.makedirs(self.config.CHECKPOINT_FOLDER)
        self._setup_actor_critic_agent(ppo_cfg)
        if self.config.RESUME_CKPT_PATH_DIR != "":
            ckpt_dict = self.load_checkpoint(self.config.RESUME_CKPT_PATH_DIR, map_location="cpu")
            print(f"resume from {self.config.RESUME_CKPT_PATH_DIR}")
            self.agent.load_state_dict(ckpt_dict["state_dict"])
            self.resume_id = int(self.config.RESUME_CKPT_PATH_DIR.split(".")[1])
            from tensorboard.backend.event_processing import event_accumulator
            # tb = event_accumulator.EventAccumulator(self.config.RESUME_TB_PATH_DIR,
            # # ea = event_accumulator.EventAccumulator('events.out.tfevents.1622089596.2015e10bcebb.1583.0',
            ...
            # })
            # tb.Reload() # loads events from file
            # self.resume_iter = tb.Scalars('train/reward')[-1][1]
        ...
        # batch_size = self.envs.batch_size
        # current_episode_reward = torch.zeros(self.envs.batch_size, 1)
        # running_episode_stats = dict(
        #     count=torch.zeros(self.envs.batch_size, 1),
        #     reward=torch.zeros(self.envs.batch_size, 1),
        # )
            ...
            for update in range(self.config.NUM_UPDATES):
                ...
                try:
                    for step in range(ppo_cfg.num_steps):
                        ...
                        env_time += delta_env_time
                        if count_steps == 0:
                            count_steps += delta_steps * (self.resume_id * self.config.CHECKPOINT_INTERVAL)
                        count_steps += delta_steps
                ...
                if update % self.config.CHECKPOINT_INTERVAL == 0:
                    self.save_checkpoint(
                        f"ckpt.{count_checkpoints+self.resume_id}.pth", dict(step=count_steps)
                    )
                    count_checkpoints += 1

                if self.config.CLEAR_INTERVAL > 0 and (update + 1) % self.config.CLEAR_INTERVAL == 0:
                    rollouts = self.reset_env(ppo_cfg)
    ...
    def _eval_checkpoint(
        ...
        num_processes = self.batch_size * self.agent_num
        test_recurrent_hidden_states = torch.zeros(
            self.actor_critic.net.num_recurrent_layers,
            # self.config.NUM_PROCESSES,
            num_processes, 
        ...
        while (
            ...
            # outputs, rewards = self.envs.step([a[0].item() for a in actions])
            # observations, _, dones, infos = [list(x) for x in zip(*outputs)]
            outputs, rewards, global_dones, check_success = self.envs.step([a[0].item() for a in actions])
            if not check_success:
                raise AssertInconsistentError
            observations, _, _, infos = [list(x) for x in zip(*outputs)] # overwrite rewards
            if infos[0]["percentage_success"] or infos[1]["percentage_success"] > 0:
                self.test_list.append((pbar.n, self.series))
                with open("success.json", "w") as f:
                    json.dump(self.test_list, f)
            if global_dones[0]:
                # for img1, img2 in zip(img1s, img2s):
                img1, img2 = infos[0], infos[1]
                color_map1 = color_top_down_map(img1)
                im1 = Image.fromarray(color_map1)
                im1.save(f"color_map_{pbar.n}_{self.series}_1.png")
                color_map2 = color_top_down_map(img2)
                im2 = Image.fromarray(color_map2)
                im2.save(f"color_map_{pbar.n}_{self.series}_2.png")
                # self.cnt += 1
                # self.cnt = 0
                self.series += 1
                # import ipdb; ipdb.set_trace()
            ...
            #     img2s = []
            # else:
            #     img1s.append(infos[0])
            #     img2s.append(infos[1])
            batch = batch_obs(observations, device=self.device)

            global_dones = torch.tensor(global_dones, dtype=torch.float, device=current_episode_reward.device).unsqueeze(1)
            global_masks = 1 - global_dones
            not_done_masks = global_masks.repeat(1, self.envs.agent_num).reshape(-1, 1)
            # not_done_masks = torch.tensor(
            #     [[0.0] if done else [1.0] for done in global_dones],
            #     dtype=torch.float,
            #     device=self.device,
            # )
            ...
            for i in range(batch_size):
                ...
                # episode ended
                # if not_done_masks[i].item() == 0:
                if global_dones[0].item() == 1:
                    pbar.update()
                    episode_stats = dict()
                    try:
                        episode_stats["reward"] = current_episode_reward[i // self.agent_num].item()
                    except:
                        import ipdb; ipdb.set_trace()
                    episode_stats.update(
                        self._extract_scalars_from_info(infos[i])
                    )
                    current_episode_reward[i // self.agent_num] = 0
                    # use scene_id + episode_id as unique id for storing stats
                    stats_episodes[
                        (
                            current_episodes[i].scene_id,
